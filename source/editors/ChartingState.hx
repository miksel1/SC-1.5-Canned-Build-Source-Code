...
class ChartingState extends MusicBeatState
{
	...
	var saveValue1:FlxButton;
	var saveValue2:FlxButton;
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var stepperBPM:FlxUINumericStepper = new FlxUINumericStepper(10, 80, 1, 1, -10000000, 10000000, 3);
		stepperBPM.value = Conductor.bpm;
		stepperBPM.name = 'song_bpm';

		var stepperSpeed:FlxUINumericStepper = new FlxUINumericStepper(10, stepperBPM.y + 35, 0.1, 1, -10000000, 10000000, 3);
	...
	function addEventsUI():Void
	{
		...
		/*FOR DEBUG PURPOSES
		FlxG.save.data.preset1stuff = null;
		FlxG.save.data.preset2stuff = null;
		*/
		saveValue1 = new FlxButton(160, 50, 'Save Value 1', function ()
			{
					trace('pushing array item 1');
					FlxG.save.data.preset1stuff.push(value1InputText.text);
			});
		...
		saveValue2 = new FlxButton(160, saveValue1.y + 75, 'Save Value 2', function ()
			{
				trace('pushing array item 2');
				FlxG.save.data.preset2stuff.push(value2InputText.text);
			});
		...
		tab_group_event.add(text);
		var text:FlxText = new FlxText(300, saveValue2.y + 80, 0, "NOTE: YOU HAVE TO\nENTER THE DEBUG\nMENU AGAIN TO\nSEE PRESETS RELOAD");
		tab_group_event.add(text);

		var deleteLastOne1:FlxButton = new FlxButton(160, saveValue2.y + 40, 'Delete Last Preset (1)', function ()
			{
					trace('pushing array item 1');
					var theShit:Int = FlxG.save.data.preset1stuff.length;
					FlxG.save.data.preset1stuff.remove(FlxG.save.data.preset1stuff[theShit - 1]);
			});

		var deleteLastOne2:FlxButton = new FlxButton(160, saveValue2.y + 80, 'Delete Last Preset (2)', function ()
			{
					trace('pushing array item 2');
					var theShit:Int = FlxG.save.data.preset2stuff.length;
					FlxG.save.data.preset2stuff.remove(FlxG.save.data.preset2stuff[theShit - 1]);
			});
		
		deleteLastOne1.setGraphicSize(80, 30);
		deleteLastOne1.updateHitbox();
			
		deleteLastOne2.setGraphicSize(80, 30);
		deleteLastOne2.updateHitbox();	

		preset2 = new FlxUIDropDownMenuCustom(160, saveValue2.y + 25, FlxUIDropDownMenuCustom.makeStrIdLabelArray(FlxG.save.data.preset2stuff, true), function(pressed:String) {
			trace('list pressed 2');
			var whatIsIt:Int = Std.parseInt(pressed);
			var arraySelectedShit:String = FlxG.save.data.preset2stuff[whatIsIt];
			value2InputText.text = arraySelectedShit;
		});
		
		preset1 = new FlxUIDropDownMenuCustom(160, saveValue1.y + 25, FlxUIDropDownMenuCustom.makeStrIdLabelArray(FlxG.save.data.preset1stuff, true), function(pressed:String) {
			trace('list pressed 1');
			var whatIsIt:Int = Std.parseInt(pressed);
			var arraySelectedShit:String = FlxG.save.data.preset1stuff[whatIsIt];
			value1InputText.text = arraySelectedShit;
		});
		...
		blockPressWhileScrolling.push(preset1);
		blockPressWhileScrolling.push(preset2);
		...
		tab_group_event.add(saveValue1);
		tab_group_event.add(saveValue2);
		tab_group_event.add(deleteLastOne1);
		tab_group_event.add(deleteLastOne2);
		tab_group_event.add(preset2);
		tab_group_event.add(preset1);

		UI_box.addGroup(tab_group_event);
	}

	function reloadEventUI() {
	...
	private function saveEvents()
	{
		...
		var eventsSong:SwagSong = {
			...
			arrowPlayerSkin: _song.arrowSkin,
			...
			event7: _song.event7,
			event7Value: _song.event7Value,
			disableGhostTapping: _song.disableGhostTapping
		};
		var json = {
			"song": eventsSong
		}
